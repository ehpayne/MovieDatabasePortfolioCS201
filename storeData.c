...
void adjustBalancesTBasic(TBasic *end, TBasic *start)
{
    ...
    while(temp != end)
    {
        if(strcmp(start->ID, temp->ID) < 0)
            temp->balance = 1;
        else if(strcmp(start->ID, temp->ID) > 0)
        {
            temp->balance = -1;
        }
        else
...
void TBasic_RestoreAVL(TBasic *ancestor, TBasic *newNode)
{
    ...
    else
    {
        ...
        /*if(newNode->right != NULL)
         {
         adjustBalancesTBasic(ancestor->parent, newNode->right);
         }
...
void rightRotateTBasic(TBasic *node)
{
    ...
    {
        TBasic *grandparent = temp->parent->parent;
...
{
    TBasic *temp = node->right;   //Hold pointer to n's right child
    node->right = temp->left;      // Move temp 's left child to right child of n
    if(temp->left != NULL)
    {
        temp->left->parent = node;// Reset the left child's parent
        
    }// If the left child does exist
    if(node->parent == NULL)       // If n was the root
    {
        tBasicsRoot = temp;      // Make temp the new root
        temp->parent = NULL;   // Root has no parent
    }
    else if(node->parent->left == node) // If n was the left child of its' parent
    {
        TBasic *grandparent = temp->parent->parent;
        temp->parent->parent->left = temp;
        temp->parent = grandparent;  // Make temp the new left child
    }
    else
    {
        TBasic *grandparent = temp->parent->parent;
    ...
    temp->left = node;         // Move n to left child of temp
    node->parent = temp;         // Reset n's parent
}
